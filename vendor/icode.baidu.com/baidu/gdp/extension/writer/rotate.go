...
import (
	...
	"icode.baidu.com/baidu/gdp/extension/fileclean"
)
...
type RotateOption struct {
	...
	// 用于控制文件落盘的间隔，若超过该时间，将强制性 Flush
	// 默认为0，不定期 Flush
	// 由于 Rotate Writer 用到 BufWriter，所以若写入量很少，内容落盘将出现延迟
	...
	// 对实际的 file writer 进行 wrap，可选
	// 如 添加 bufio.NewWriter 的封装以提升写入效率
	// 默认会使用 LineBuffer
	NewWriter func(w io.Writer) RotateOptionWriter
}

// RotateOptionWriter RotateOption 所依赖的 writer 接口定义
// 	bufio.Writer 是满足该接口定义的
type RotateOptionWriter interface {
	io.Writer
	Flush() error
	Buffered() int
}
...
func (f *rotateWriter) checkOpened(info RotateInfo) (errResult error) {
	...
	if needNew {
		...
		f.outFile = logFile
		f.bufFile = f.newFileWriter(f.outFile)
	}

	return f.checkSymlink(info)
}

func (f *rotateWriter) newFileWriter(w *os.File) RotateOptionWriter {
	if f.opt.NewWriter != nil {
		return f.opt.NewWriter(w)
	}
	return NewLineBuffer(w)
}
